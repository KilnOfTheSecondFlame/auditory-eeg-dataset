...
if __name__ == "__main__":
    ...
    data_folder = os.path.join(
        config["dataset_folder"], config["derivatives"], config["split_folder"]
    )
    ...
    if only_evaluate:
        ...
        train_generator = MatchMismatchDataGenerator(
            train_files, window_length, spacing=spacing
        )
        ...
        val_generator = MatchMismatchDataGenerator(
            val_files, window_length, spacing=spacing
        )
    ...
    for window_length in window_lengths:
        ...
        for sub in subjects:
            ...
            test_generator = MatchMismatchDataGenerator(
                files_test_sub, window_length, spacing=spacing
            )
        ...
        results_path = os.path.join(
            results_folder,
            results_filename.split(".")[0] + f"_{window_length}" + ".json",
        )
