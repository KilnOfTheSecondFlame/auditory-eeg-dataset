...
if plot_dilation:
    ...
    files = glob.glob(
        os.path.join(
            base_results_folder, "results_dilated_convolutional_model/eval_*.json"
        )
    )
    # sort the files
    files.sort()
    ...
    plt.savefig(
        os.path.join(base_results_folder, "figures", "boxplot_dilated_conv.pdf")
    )

if plot_linear_backward:
    ## linear backward regression plots
    # load the results
    results_files_glob = os.path.join(
        base_results_folder, "results_linear_backward", "eval*.json"
    )
    ...
    print(
        "Found {} subjects, {} of which had at least one significant result".format(
            nb_subjects, nb_significant
        )
    )
    ...
    print(
        "Found {} recordings, {} of which were significant".format(
            nb_recordings, nb_significant_recordings
        )
    )
    non_significant_recordings_series = (
        df.groupby(["subject", "stim"]).agg({"significant": "any"})["significant"]
        == False
    )
    non_significant_recordings = non_significant_recordings_series[
        non_significant_recordings_series
    ].index
    ...
    for band_name, (highpass, lowpass) in freq_bands.items():
        ...
        values_for_boxplot.append(
            selected_df.groupby("subject").agg("mean")["score"].tolist()
        )
    ...
    temp_df = pd.DataFrame(
        values_for_boxplot, index=freq_bands.keys(), columns=range(85)
    ).T
    ...
    plt.savefig(
        os.path.join(
            base_results_folder, "figures", "boxplot_linear_backward_frequency.pdf"
        )
    )
    # plt.close()
    ...
    fig, (ax0, ax1) = plt.subplots(
        1, 2, width_ratios=[0.2, 0.8], figsize=(15, 6), sharey=True
    )

    for stimulus in sorted(pd.unique(df["stim"]), key=sort_key_fn):
        selected_df = df[
            (df["stim"] == stimulus) & (df["highpass"] == 0.5) & (df["lowpass"] == 4.0)
        ]
        values_for_boxplot.append(
            selected_df.groupby("subject").agg("mean")["score"].tolist()
        )
    ...
    for stimulus_type, stimulus_selector in stim_type_selectors.items():
        selected_df = df[
            ...
            & (df["highpass"] == 0.5)
            & (df["lowpass"] == 4.0)
            & (~(df["stim"].str.endswith("artefact")))
            & (~(df["stim"].str.endswith("shifted")))
            & (~(df["stim"].str.endswith("audiobook_1_1")))
            & (~(df["stim"].str.endswith("audiobook_1_2")))
        ]
        values_for_boxplot.append(
            selected_df.groupby("subject").agg("mean")["score"].tolist()
        )
    ...
    print(
        "MannWhitneyU test: {}, medians: {}, {}".format(
            scipy.stats.mannwhitneyu(test_data[0], test_data[1]),
    ...
    plt.savefig(
        os.path.join(
            base_results_folder,
            "figures",
            "boxplot_linear_backward_stimulus_combined.pdf",
        )
    )
...
if plot_linear_forward:
    results_files_glob = os.path.join(
        base_results_folder, "results_linear_forward", "eval*.json"
    )
    results_files = glob.glob(results_files_glob)
    all_results = []
    ...
    info = mne.create_info(
        ch_names=montage.ch_names, sfreq=sfreq, ch_types="eeg"
    ).set_montage(montage)

    fig, axes = plt.subplots(3, 5, figsize=(16, 12))
    temp_df = df.copy()
    temp_df["filterband"] = (
        temp_df["highpass"].astype(str) + "-" + temp_df["lowpass"].astype(str)
    )
    scores = temp_df.groupby(["filterband"]).agg(
        {"scores_per_channel": lambda x: list(x)}
    )
    ...
    for index, (stim_type, selector) in enumerate(stim_type_selectors.items()):
        ...
        for index2, (band_name, (highpass, lowpass)) in enumerate(freq_bands.items()):
            ...
            selected_df = df[
                (df["highpass"] == highpass) & (df["lowpass"] == lowpass) & (selector)
            ]
            scores_per_subject = selected_df.groupby("subject").agg(
                {"scores_per_channel": lambda x: np.mean(np.stack(x, axis=1), axis=1)}
            )
    ...
    fig.suptitle(
        "Forward model performance across stimuli types and frequency bands",
        y=0.94,
        fontsize=18,
    )
    fig.tight_layout()
    ...
    plt.savefig(
        os.path.join(base_results_folder, "figures", f"topoplot_linear_forward.pdf")
    )
    # plt.show()
    plt.close(fig)
...
if plot_vlaai:
    ...
    results_files = glob.glob(
        os.path.join(base_results_folder, "results_vlaai/eval*.json")
    )
